function fulltimeline_ch() {
 
    var width = document.getElementById("fulltimeline_china").offsetWidth,
        height = document.getElementById("fulltimeline_china").offsetHeight,
        margin = 10;
    
    d3.selectAll(".fulltimeline_china").remove()
    
    var parseTime = d3.timeParse("%Y%m%d");

    var tooltip = d3.select("#fulltimeline_china")
        .append("div")
        .attr("id", "tooltip")
        .style("opacity", 0);

    var svg = d3.select("#fulltimeline_china")
        .attr("class", "fulltimeline ft")
        .append("svg")
        .attr("width", width)
        .attr("height", height)
...
/*    var selected = $("input[type='radio'][name='countries']:checked").val();*/
    ...
    d3.csv("data/ssp_simplified.csv").then(function (data) {
        ...
        var node = svg.append("g")
            ...
            .attr("r", function (d) {
                return violenceScale(d.violence_policeorstate)/2;
            })
            .attr("cx", width / 2)
            .attr("cy", height / 2)
